# The gain for the angular velocity (kp_angular) is not a 
# standard parameter in the pure pursuit algorithm.
# The pure pursuit algorithm directly computes the 
# curvature needed to follow the path, and the angular velocity 
# is typically derived from this curvature without additional proportional gain tuning. 
# Here's how the algorithm typically works:

# - Lookahead Point: Find the lookahead point on the path at a distance ld (lookahead distance) 
# from the robot's current position.
# - Curvature Calculation: Calculate the curvature needed to reach the lookahead point.
# - Velocity Commands: Use the curvature to compute the angular velocity command directly.
# Can use
#!/usr/bin/env python3
