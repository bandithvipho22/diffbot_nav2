...
# def smooth_trajectory(trajectory_x, trajectory_y):
#     control_points = list(zip(trajectory_x, trajectory_y))
#     return bspline_planning(control_points)

def smooth_trajectory(trajectory_x, trajectory_y, window_size=20, poly_order=2):
    smoothed_x = bspline_planning(trajectory_x, window_size, poly_order)
    smoothed_y = bspline_planning(trajectory_y, window_size, poly_order)
    return savgol_filter(smoothed_x, smoothed_y)
...
class PathPublisherNode(Node):
    def __init__(self):
        ...
        self.laser_subscription = self.create_subscription(LaserScan, '/scan', self.laser_callback, 1)
