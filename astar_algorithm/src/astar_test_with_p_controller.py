...
class PathPublisherNode(Node):
    ...
    def p_timer_callback(self):
        direction = [self.rx[0] - self.sx, self.ry[0] - self.sy]
        velocity = math.hypot(direction[0], direction[1])
        theta_difference = math.atan2(direction[1], direction[0]) - self.yaw
        ...
        if abs(theta_difference) > 0.1:
            cmd_msg.linear.x = 0.0
            cmd_msg.angular.z = kp_w * theta_difference
        elif abs(theta_difference) < 0.1:
        ...
        #print(self.rx)
        print(direction, theta_difference)
