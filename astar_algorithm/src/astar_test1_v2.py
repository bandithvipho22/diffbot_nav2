...
class AStarPublisher(Node):
    ...
    def publish_path(self):
        ...
        robot_radius = 1.0
         ...
         # Create obstacle map
        obstacle_map = self.calc_obstacle_map(ox, oy, resolution, robot_radius)
    ...
    class AStarPlanner:
        ...
        def __init__(self, sx, sy, gx, gy, resolution, rr):
            self.sx = sx
            self.sy = sy
            self.gx = gx
            self.gy = gy
            ...
            self.rr = rr
            self.min_x = min(sx, gx) - rr
            self.min_y = min(sy, gy) - rr
            self.max_x = max(sx, gx) + rr
            self.max_y = max(sy, gy) + rr
        ...
        #-----------------------------------------
        def calc_obstacle_map(self, ox, oy, resolution, robot_radius):
            x_min, y_min = min(ox) - robot_radius, min(oy) - robot_radius
            x_max, y_max = max(ox) + robot_radius, max(oy) + robot_radius
            x_range = int((x_max - x_min) / resolution)
            y_range = int((y_max - y_min) / resolution)

            obstacle_map = [[0 for _ in range(y_range)] for _ in range(x_range)]

            for x, y in zip(ox, oy):
                x_idx = int((x - x_min) / resolution)
                y_idx = int((y - y_min) / resolution)
                obstacle_map[x_idx][y_idx] = 1
