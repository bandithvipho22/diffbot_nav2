...
class P_Path(Node):
    def __init__(self):
        ...
        self.i = 0  # Initialize the path index
    ...
    def odom_callback(self, msg):
        ...
        if abs(theta_difference) > 1.0:
            twist.linear.x = 0.0
            twist.angular.z = self.lookahead_distance * theta_difference
        elif abs(theta_difference) < 1.0:
            twist.linear.x = kp_v * velocity
            twist.angular.z = self.lookahead_distance * theta_difference
        ...
        if len(self.rx) > 0 and len(self.ry) > 0:  # Ensure rx and ry are not empty
            if distance(self.sx, self.sy, self.rx[0], self.ry[0]) < 0.2:
                ...
                self.get_logger().info(f'Theta difference: {theta_difference}')
                if theta_difference > pi:
                    theta_difference -= 2 * pi
                elif theta_difference < -pi:
                    theta_difference += 2 * pi
                ...
                if abs(theta_difference) > 0.5:
                    twist.angular.z = theta_difference
                else:
                    twist.angular.z = 0.0
