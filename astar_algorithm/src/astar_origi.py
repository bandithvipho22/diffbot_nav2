...
class PathPublisherNode(Node):
    def __init__(self):
        ...
        # self.estimate_publish = self.create_publisher(PoseWithCovarianceStamped, '/amcl', 1)
    ...
    def laser_callback(self, msg):
        ...
        self.ox = np.concatenate((self.map_ox, self.laser_ox))
        self.oy = np.concatenate((self.map_oy, self.laser_oy))
        self.ox, self.oy = remove_close_obstacles(self.ox, self.oy, 0.08)
    ...
    def pose_estimate_callback(self, msg):
        ...
        # self.get_logger().info('x = %f, y = : %f' % (self.estimate_odom_x,self.estimate_odom_y ))
        # self.get_path()
        # estimate_odom = PoseWithCovarianceStamped()
        # estimate_odom.header.stamp = self.get_clock().now().to_msg()
        # estimate_odom.header.frame_id = 'map'
        # estimate_odom.pose.pose.position.x = self.estimate_odom_x 
        # estimate_odom.pose.pose.position.y =  self.estimate_odom_y
        # estimate_odom.pose.pose.orientation = msg.pose.pose.orientation
        # self.get_logger().info('x = %f, y = : %f' % (estimate_odom.pose.pose.position.x ,estimate_odom.pose.pose.position.y))
        # self.estimate_publish.publish(estimate_odom)
    ...
    def odom_callback(self, msg):
        ...
        # if self.estimate_odom_x and self.estimate_odom_y and self.start is not None:
        #     self.sx = self.estimate_odom_x
        #     self.sy = self.estimate_odom_y
        #     self.gx = self.estimate_odom_x
        #     self.gy = self.estimate_odom_y
        #     self.get_path()
        #     self.start = None
        #     self.get_logger().info('x2 = %f, y2 = : %f' % (self.sx,self.sy ))
    ...
    def get_path(self):
        ...
        num_segments = int(np.ceil(initial_length / 0.07))
    ...
    def map_callback(self, msg):
        ...
        self.map_ox, self.map_oy = remove_close_obstacles(self.map_ox, self.map_oy, 0.7)
