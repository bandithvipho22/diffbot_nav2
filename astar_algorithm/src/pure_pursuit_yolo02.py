...
from yolov8_msgs.msg import ObjectDistance  # Assuming this is your custom message type for object distance
...
class P_Path(Node):
    def __init__(self):
        ...
        self.distance_sub = self.create_subscription(ObjectDistance, '/object_distance', self.distance_callback, 10)
        ...
        self.stop_sign_detected = False  # Flag for stop sign detection
        ...
        self.stop_start_time = None  # Variable to store stop start time

    def distance_callback(self, msg):
        if msg.class_name == "stop sign" and not self.stop_sign_detected:
            self.stop_sign_detected = True
            self.stop_start_time = time.time()  # Record the time when the stop sign is detected
    ...
    def odom_callback(self, msg):
        ...
        twist = Twist()
        twist.angular.z = curvature
        
        # Linear Velocity Control
        direction = hypot(self.gx - self.sx, self.gy - self.sy)
        velocity_error = direction
        kp_v = 2.0
        
        if abs(alpha) > 1.0 or self.stop_sign_detected:
            twist.linear.x = 0.0
